...
`application.properties` ファイルは下図の箇所から編集できます。
...
* Cliend ID: quarkus
* Root URL: http://people-{{ USER_ID }}-keycloak.apps.{{ ROUTE_SUBDOMAIN }}/
...
image::terminal2.png[terminal1, 800]

OpenShiftの開発者者向け表示のトポロジーを確認すると、Quakursがデプロイされていることがわかります。

image::quarkus_topology.png[terminal1, 800]
...
* http://people-{{ USER_ID }}-keycloak.apps.{{ ROUTE_SUBDOMAIN }}/tokens
