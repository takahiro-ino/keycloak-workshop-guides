...
VS Codeの多くの機能には、コマンド パレットからアクセスします。「Get Started」ページでいくつかのチュートリアルを参照できます(例: OpenShift Cluster のログイン/プロビジョニング、Devfile レジストリーからのコンポーネントの作成 Webviewなど)。
...
このクラスには、/tokensのRESTfulエンドポイント定義があります。
